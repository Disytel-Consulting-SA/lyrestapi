...
public final class WindowFieldDefaultResolver {
    ...
    private static final CLogger log = CLogger.getCLogger(WindowFieldDefaultResolver.class);
    ...
    public static String resolve(UserInfo info, Integer adReferenceId, String columnName, boolean isKey, boolean isParent, String defaultValue) {

        /*
         * MField.getDefault() no genera defaults
         * para keys, RowID ni Binary.
         */
        if (isKey || adReferenceId == DisplayType.RowID || adReferenceId == DisplayType.Binary) {
            return null;
        }
        ...
        if (!isEmpty(defaultValue)) {
            String resolved = resolveExplicitDefault(ctx, columnName, defaultValue);
            String normalized = normalize(columnName, adReferenceId, resolved);
        ...
        // Button -> N
        if (adReferenceId == DisplayType.Button && columnName != null && !columnName.endsWith("_ID")) {
            return "N";
        }
        ...
        // Valores numéricos -> 0
        if (adReferenceId != null && DisplayType.isNumeric(adReferenceId)) {
            return "0";
        }
    ...
    private static String resolveExplicitDefault(Properties ctx, String columnName, String defaultValue) {
        ...
        if (defaultValue.startsWith("@SQL=")) {
            return resolveSql(ctx, columnName, defaultValue);
        }
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            String resolved = resolveToken(ctx, token);
     ...
     */
    private static String resolveToken(Properties ctx, String token) {
         ...
         */
        if (token.indexOf('@') != -1) {
            String variable = token.replace('@', ' ').trim();
            String value = Env.getContext(ctx, WINDOW_NO, variable);
     ...
     */
    private static String normalize(String columnName, Integer adReferenceId, String value) {
        ...
        try {
             ...
             */
            if ((columnName != null && (columnName.endsWith("atedBy") || columnName.endsWith("_ID"))) || adReferenceId == DisplayType.Integer) {
             ...
             */
            log.log(Level.FINE, "DefaultValue inválido para " + columnName + ": " + value);
            return null;
        }
    }
     ...
     */
    private static String resolveSql(Properties ctx, String columnName, String defaultValue) {

        String sql = defaultValue.substring(5);
        sql = Env.parseContext(ctx, WINDOW_NO, sql, false, true);

        if (isEmpty(sql)) {
            log.log(
                    Level.WARNING,
                    "No fue posible resolver variables del DefaultValue SQL para columna " + columnName + ": " + defaultValue
            );
            return null;
        }
        ...
        try {
            ...
            log.log(Level.WARNING, "Error resolviendo DefaultValue SQL para columna " + columnName + ": " + defaultValue, e);
        } finally {
            DB.close(rs, ps);
        }
