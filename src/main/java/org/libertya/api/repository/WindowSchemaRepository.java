...
public class WindowSchemaRepository {
    ...
    private List<String> getPkColumns(String tableName, String dataEndpoint) {
        if (dataEndpoint == null) return null;
        ...
        if (pkColumns != null) return Arrays.asList(pkColumns);
    ...
    public WindowSchema retrieve(Integer windowId) {
        return retrieve(null, windowId, null);
    }

    /**
     * Mantiene compatibilidad con llamadas sin contexto.
     */
    public WindowSchema retrieve(Integer windowId, String language) {
        return retrieve(null, windowId, language);
    }
     ...
     */
    public WindowSchema retrieve(UserInfo info, Integer windowId, String language) {
         ...
         * se solicita idioma.
         */
        String windowName = translated ? "COALESCE(wt.name, w.name)" : "w.name";
        String windowDescription = translated ? "COALESCE(wt.description, w.description)" : "w.description";
        String tabName = translated ? "COALESCE(tt.name, t.name)" : "t.name";
        ...
        sql.append("   ").append(tabDescription).append(" AS tab_description, ");
         ...
         */
        sql.append("   tc.columnname AS tab_link_columnname, ");

        sql.append("   tb.ad_table_id, ");
        sql.append("   tb.tablename, ");

        sql.append("   f.ad_field_id, ");
        sql.append("   ").append(fieldName).append(" AS field_name, ");
         ...
         */
        if (translated) {
            sql.append(" LEFT JOIN ad_window_trl wt ON wt.ad_window_id = w.ad_window_id AND wt.ad_language = ? ");
        }

        sql.append(" JOIN ad_tab t ON t.ad_window_id = w.ad_window_id ");

        /*
         * Traducción de pestaña.
         */
        if (translated) {
            sql.append(" LEFT JOIN ad_tab_trl tt ON tt.ad_tab_id = t.ad_tab_id AND tt.ad_language = ? ");
        }
         ...
         */
        sql.append(" LEFT JOIN ad_column tc ON tc.ad_column_id = t.ad_column_id ");

        sql.append(" WHERE w.ad_window_id = ? ");
        sql.append("   AND w.isactive = 'Y' ");
        sql.append("   AND t.isactive = 'Y' ");
        sql.append("   AND f.isactive = 'Y' ");
        sql.append("   AND c.isactive = 'Y' ");

        sql.append(" ORDER BY t.seqno, f.seqno ");
        ...
        try {
             ...
             */
            List<WindowSchemaTab> createdTabs = new ArrayList<>();
            ...
            while (rs.next()) {
                ...
                if (!tabId.equals(currentTabId)) {
                    ...
                    currentTab = new WindowSchemaTab()
                            ...
                            .description(rs.getString("tab_description"))
                            .seqno(rs.getInt("tab_seqno"))
                            .tablevel(tablevel)
                     ...
                     */
                    if (tablevel > 0) {
                        Integer parentTabId = findParentTabId(createdTabs, tablevel);
                        if (parentTabId != null) {
                            currentTab.parentAdTabId(parentTabId);
                        }
                    }
                     ...
                     */
                    String explicitLinkColumn = rs.getString("tab_link_columnname");
                    if (explicitLinkColumn != null && !explicitLinkColumn.isEmpty()) {
                        explicitLinkColumns.put(tabId, explicitLinkColumn);
                    }
                 ...
                 */
                WindowSchemaField field = new WindowSchemaField()
                        ...
                        .defaultvalue(WindowFieldDefaultResolver.resolve(
                                ...
                                "Y".equals(rs.getString("isparent")),
            ...
            if (schema != null) {
                 ...
                 */
                resolveLinkColumns(schema, explicitLinkColumns);
                 ...
                 */
                resolveListReferences(schema, effectiveLanguage);
            ...
            throw new RuntimeException(
                    "Error recuperando schema de ventana " + windowId + (translated ? " para idioma " + effectiveLanguage : ""),
                    e
            );
        } finally {
            DB.close(rs, ps);
    ...
    /**
     * Carga el mapping tabla -> endpoint REST generado por utils/genSchema.sh.
     */
    private static Properties loadTableEndpoints() {
        Properties properties = new Properties();

        try (InputStream input = WindowSchemaRepository.class.getClassLoader().getResourceAsStream("table-endpoints.properties")) {
            if (input == null) {
                throw new IllegalStateException("No se encontró table-endpoints.properties en el classpath");
            }
     ...
     */
    private Integer findParentTabId(List<WindowSchemaTab> tabs, int currentLevel) {
        int parentLevel = currentLevel - 1;

        for (int i = tabs.size() - 1; i >= 0; i--) {
            WindowSchemaTab candidate = tabs.get(i);
            if (candidate.getTablevel() != null && candidate.getTablevel().intValue() == parentLevel) {
     ...
     */
    private void resolveLinkColumns(WindowSchema schema, Map<Integer, String> explicitLinkColumns) {
        if (schema.getTabs() == null) return;

        List<WindowSchemaTab> tabs = schema.getTabs();

        for (int tabIndex = 0; tabIndex < tabs.size(); tabIndex++) {
            WindowSchemaTab tab = tabs.get(tabIndex);

            if (tab.getTablevel() == null || tab.getTablevel() == 0) {
                continue;
            }
             ...
             */
            String explicitColumn = explicitLinkColumns.get(tab.getAdTabId());
            if (explicitColumn != null && !explicitColumn.isEmpty()) {
                tab.linkColumnname(explicitColumn);
                continue;
            }
             ...
             */
            List<WindowSchemaField> parentFields = getParentFields(tab);

            /*
             * 2. Un único candidato.
             */
            if (parentFields.size() == 1) {
                tab.linkColumnname(parentFields.get(0).getColumnname());
                continue;
            }
             ...
             */
            if (parentFields.size() > 1) {
                String linkColumn = resolveLinkFromPreviousTabs(tabs, tabIndex, parentFields);
                if (linkColumn != null) {
                    tab.linkColumnname(linkColumn);
                }
            }
        }
    }
     ...
     */
    private List<WindowSchemaField> getParentFields(WindowSchemaTab tab) {
        List<WindowSchemaField> result = new ArrayList<>();
        if (tab.getFields() == null) return result;

        for (WindowSchemaField field : tab.getFields()) {
            if (Boolean.TRUE.equals(field.isIsparent())) {
                result.add(field);
            }
        }
        return result;
    ...
    /**
     * Resuelve una columna master/detail cuando existen múltiples candidatos IsParent.
     */
    private String resolveLinkFromPreviousTabs(List<WindowSchemaTab> tabs, int currentTabIndex, List<WindowSchemaField> parentFields) {
        for (int i = currentTabIndex - 1; i >= 0; i--) {
            WindowSchemaTab previousTab = tabs.get(i);
            if (previousTab.getFields() == null) continue;

            for (WindowSchemaField previousField : previousTab.getFields()) {
                if (!Boolean.TRUE.equals(previousField.isIskey())) continue;

                String keyColumn = previousField.getColumnname();
                if (keyColumn == null) continue;

                for (WindowSchemaField parentField : parentFields) {
                    String parentColumn = parentField.getColumnname();
                    if (parentColumn != null && keyColumn.equalsIgnoreCase(parentColumn)) {
                        return parentColumn;
                    }
                }
            }
     ...
     * Se recuperan todos los AD_Reference_ID utilizados y luego todos los valores mediante una única consulta.
     */
    private void resolveListReferences(WindowSchema schema, String language) {
        Set<Integer> referenceIds = new LinkedHashSet<>();

        /*
         * Primera pasada: determinar qué listas necesita esta ventana.
         */
        for (WindowSchemaTab tab : schema.getTabs()) {
            if (tab.getFields() == null) continue;
            ...
            for (WindowSchemaField field : tab.getFields()) {
                if (field.getAdReferenceId() != null
                        && field.getAdReferenceId() == REFERENCE_LIST
                        && field.getAdReferenceValueId() != null
                        && field.getAdReferenceValueId() > 0) {
                    referenceIds.add(field.getAdReferenceValueId());
                }
            }
        }
        ...
        /*
         * Recuperar todos los valores de todas las listas en una sola query.
         */
        Map<Integer, List<WindowSchemaReferenceValue>> valuesByReference = loadListReferenceValues(referenceIds, language);

        /*
         * Segunda pasada: asociar a cada field su definición semántica.
         */
        for (WindowSchemaTab tab : schema.getTabs()) {
            if (tab.getFields() == null) continue;

            for (WindowSchemaField field : tab.getFields()) {
                if (field.getAdReferenceId() == null || field.getAdReferenceId() != REFERENCE_LIST) {
                    continue;
                }

                WindowSchemaReference reference = new WindowSchemaReference().type("list");
                List<WindowSchemaReferenceValue> values = valuesByReference.get(field.getAdReferenceValueId());

                if (values != null) {
                    for (WindowSchemaReferenceValue value : values) {
     ...
     */
    private void resolveBooleanReferences(WindowSchema schema) {
        if (schema.getTabs() == null) return;
        ...
        for (WindowSchemaTab tab : schema.getTabs()) {
            ...
            for (WindowSchemaField field : tab.getFields()) {
                if (field.getAdReferenceId() != null && field.getAdReferenceId() == REFERENCE_YESNO) {
                    field.reference(new WindowSchemaReference().type("boolean"));
                }
            }
        }
    }
     ...
     * Search utiliza el mismo endpoint de resolución, pero el frontend lo presenta mediante ventana modal.
     */
    private void resolveLookupReferences(WindowSchema schema) {
        if (schema.getTabs() == null) return;
        ...
        for (WindowSchemaTab tab : schema.getTabs()) {
            ...
            for (WindowSchemaField field : tab.getFields()) {
                ...
                String endpoint = "/v1.0/columns/" + field.getAdColumnId() + "/lookup";

                /*
                 * Table / Table Direct
                 */
                if (referenceId == REFERENCE_TABLE || referenceId == REFERENCE_TABLE_DIRECT) {
                    field.reference(new WindowSchemaReference().type("lookup").endpoint(endpoint));
                    continue;
                }
                ...
                if (referenceId == REFERENCE_SEARCH) {
                    field.reference(new WindowSchemaReference().type("search").endpoint(endpoint));
                }
            }
        }
    }
    ...
    /**
     * Recupera en una única consulta los valores correspondientes a múltiples AD_Reference_ID.
     * Si language viene informado, intenta obtener Name desde AD_Ref_List_Trl.
     * Si no existe traducción, se utiliza rl.Name.
     */
    private Map<Integer, List<WindowSchemaReferenceValue>> loadListReferenceValues(Set<Integer> referenceIds, String language) {
        Map<Integer, List<WindowSchemaReferenceValue>> result = new HashMap<>();
        boolean translated = language != null && !language.trim().isEmpty();
        ...
        String listName = translated ? "COALESCE(rlt.name, rl.name)" : "rl.name";

        StringBuilder sql = new StringBuilder();
        sql.append(" SELECT rl.ad_reference_id, rl.value, ").append(listName).append(" AS name ");
        sql.append(" FROM ad_ref_list rl ");

        if (translated) {
            sql.append(" LEFT JOIN ad_ref_list_trl rlt ON rlt.ad_ref_list_id = rl.ad_ref_list_id AND rlt.ad_language = ? ");
        }

        sql.append(" WHERE rl.isactive = 'Y' ");
        sql.append("   AND rl.ad_reference_id IN (").append(placeholders).append(") ");
        sql.append(" ORDER BY rl.ad_reference_id, name ");
        ...
        try {
            ...
            /*
             * El parámetro del JOIN aparece antes que los IDs del IN.
             */
            if (translated) {
                ps.setString(parameterIndex++, language.trim());
            }

            for (Integer referenceId : referenceIds) {
                ps.setInt(parameterIndex++, referenceId);
            }
            ...
            while (rs.next()) {
                ...
                WindowSchemaReferenceValue value = new WindowSchemaReferenceValue()
            ...
            throw new RuntimeException(
                    "Error recuperando valores de AD_Ref_List" + (translated ? " para idioma " + language : ""),
                    e
            );
        } finally {
            DB.close(rs, ps);
    ...
    private void resolveVisualReferences(WindowSchema schema) {
        if (schema.getTabs() == null) return;
