...
public interface OrderApi {
    ...
    @ApiOperation(value = "Retrieve orders", nickname = "getAllOrders", notes = "Retorna una lista de pedidos", response = Order.class, responseContainer = "List", tags={ "order", })
    @ApiResponses(value = { 
...
,@NotNull @ApiParam(value = "Accion a aplicar (completar, revertir, etc.)", required = true) @Valid @RequestParam(value = "action", required = true) String action
    ...
    @ApiOperation(value = "Recupera un pedido en particular", nickname = "retrieveOrder", notes = "Recupera la informacion de un pedido en particular", response = OrderDocument.class, tags={ "order", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = OrderDocument.class) })
    @RequestMapping(value = "/v1.0/orders/{id}",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    default ResponseEntity<OrderDocument> retrieveOrder(@ApiParam(value = "ID del pedido",required=true) @PathVariable("id") Integer id
) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
