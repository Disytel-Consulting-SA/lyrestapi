...
public interface MessageApi {
    ...
    @Operation(summary = "Recupera un mensaje en particular", description = "Recupera la informacion de un mensaje en particular", security = {
        @SecurityRequirement(name = "jwtAuth")    }, tags={ "message" })
    @ApiResponses(value = { 
    ...
    @RequestMapping(value = "/v1.0/messages/{id}",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    default ResponseEntity<Message> retrieveMessage(@Parameter(in = ParameterIn.PATH, description = "ID del mensaje", required=true, schema=@Schema()) @PathVariable("id") Integer id) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
